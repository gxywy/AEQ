...
from algos.core import TanhGaussianPolicy, Mlp, soft_update_model1_with_model2, ReplayBuffer,\
...
class UFQSACAgent(object):
    ...
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
                 ...
                 policy_update_delay=20, base_beta=0.5, scale_beta=0.5, prior_net=False,
                 ):
        # set up networks
        ...
        if prior_net:
            ...
            for prior_i in range(num_Q):
                new_prior_net = Mlp(obs_dim + act_dim, 1, hidden_sizes).to(device)
                self.prior_net_list.append(new_prior_net)
                new_prior_target_net = Mlp(obs_dim + act_dim, 1, hidden_sizes).to(device)
                new_prior_target_net.load_state_dict(new_prior_net.state_dict())
    ...
    def get_ave_q_prediction_for_bias_evaluation(self, obs_tensor, acts_tensor):
        ...
        for q_i in range(self.num_Q):
            if self.prior_net:
                q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1)) + self.prior_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1))
            else:
    ...
    def sample_data(self, batch_size):
        ...
        count_tensor = Tensor(batch['count']).unsqueeze(1).to(self.device)
        return obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor, count_tensor

    def get_redq_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor, count_tensor):
        ...
        with torch.no_grad():
            if self.q_target_mode == 'min':
                ...
                for sample_idx in sample_idxs:
                    if self.prior_net:
                        q_prediction_next = self.q_target_net_list[sample_idx](torch.cat([obs_next_tensor, a_tilda_next], 1)) + self.prior_target_net_list[sample_idx](torch.cat([obs_next_tensor, a_tilda_next], 1))
                    else:
            ...
            if self.q_target_mode == 'ave':
                ...
                for q_i in range(self.num_Q):
                    if self.prior_net:
                        q_prediction_next = self.q_target_net_list[q_i](torch.cat([obs_next_tensor, a_tilda_next], 1)) + self.prior_target_net_list[q_i](torch.cat([obs_next_tensor, a_tilda_next], 1))
                    else:
            ...
            if self.q_target_mode == 'ufq':
                ...
                for q_i in range(self.num_Q):
                    ...
                    q_prediction_next_list.append(q_prediction_next)
                q_prediction_next_ave = torch.cat(q_prediction_next_list, 1).mean(dim=1).reshape(-1, 1)
                q_prediction_next_std = torch.cat(q_prediction_next_list, 1).std(dim=1).reshape(-1, 1)
                count_beta = (count_tensor - 1) / self.replay_buffer.max_count
                count_beta = self.base_beta + self.scale_beta * count_beta
                next_q_with_log_prob = (q_prediction_next_ave - count_beta * q_prediction_next_std) - self.alpha * log_prob_a_tilda_next
                y_q = rews_tensor + self.gamma * (1 - done_tensor) * next_q_with_log_prob
    ...
    def train(self, logger):
        ...
        for i_update in range(num_update):
            obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor, count_tensor = self.sample_data(self.batch_size)

            """Q loss"""
            y_q, sample_idxs = self.get_redq_q_target_no_grad(obs_next_tensor, rews_tensor, done_tensor, count_tensor)
            q_prediction_list = []
            for q_i in range(self.num_Q):
                if self.prior_net:
                    q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1)) + self.prior_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1)).detach()
                else:
                    q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1))
                q_prediction_list.append(q_prediction)
            q_prediction_cat = torch.cat(q_prediction_list, dim=1)
            ...
            if ((i_update + 1) % self.policy_update_delay == 0) or i_update == num_update - 1:
                ...
                for sample_idx in range(self.num_Q):
                    ...
                    if self.prior_net:
                        self.prior_net_list[sample_idx].requires_grad_(False)
                        q_a_tilda = self.q_net_list[sample_idx](torch.cat([obs_tensor, a_tilda], 1)) + self.prior_net_list[sample_idx](torch.cat([obs_tensor, a_tilda], 1))
                    else:
                ...
                for sample_idx in range(self.num_Q):
                    ...
                    if self.prior_net:
                        self.prior_net_list[sample_idx].requires_grad_(True)
