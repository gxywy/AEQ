...
import time
import sys
from algos.ufq_sac import UFQSACAgent
from algos.core import mbpo_epoches, test_agent
from utils.run_utils import setup_logger_kwargs
from utils.bias_utils import log_bias_evaluation
from utils.logx import EpochLogger
from rl_plotter.logger import Logger
import os.path as osp

def ufq_sac(env_name, seed=0, epochs='mbpo', steps_per_epoch=1000,
             ...
             utd_ratio=20, num_Q=10, num_min=2, q_target_mode='ufq',
             policy_update_delay=20,
             # following are bias evaluation related
             evaluate_bias=True, n_mc_eval=1000, n_mc_cutoff=350, reseed_each_epoch=True,
             # following are ufq related
             mylogger=None, base_beta=0.5, scale_beta=0.5, prior_net=False,
             ):
    """
    ...
    agent = UFQSACAgent(env_name, obs_dim, act_dim, act_limit, device,
                 hidden_sizes, replay_size, batch_size,
                 lr, gamma, polyak,
                 alpha, auto_alpha, target_entropy,
                 start_steps, delay_update_steps,
                 utd_ratio, num_Q, num_min, q_target_mode,
                 policy_update_delay, base_beta, scale_beta, prior_net)
    ...
    for t in range(total_steps):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            ep_return_list = test_agent(agent, test_env, max_ep_len, logger, n_eval=1) # add logging here
            mylogger.update(ep_return_list, t + 1)
            if evaluate_bias:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='ufq_sac')
    parser.add_argument('--data_dir', type=str, default='./data/')
    parser.add_argument('--debug', action='store_true')
    # Additional arguments
    parser.add_argument("--utd_ratio", default=20)
    parser.add_argument("--policy_delay", type=int, default=20)
    parser.add_argument("--replay_size", type=int, default=1000000)
    parser.add_argument("--base_beta", type=float, default=0.5)
    ...
    # python train_ufq_sac.py --env Ant-v2 --seed 0 --base_beta 0.3 --scale_beta 0.1 --ep_seed
    ...
    mylogger = Logger(log_dir=osp.join(args.data_dir, exp_name_full), exp_name=exp_name_full, env_name=args.env, seed=args.seed, config=locals())

    ufq_sac(args.env, seed=args.seed, epochs=args.epochs,
             logger_kwargs=logger_kwargs, debug=args.debug, replay_size=args.replay_size,
             evaluate_bias=args.log_q, utd_ratio=args.utd_ratio, reseed_each_epoch=args.ep_seed, policy_update_delay=args.policy_delay,
             mylogger=mylogger, base_beta=args.base_beta, scale_beta=args.scale_beta, 
